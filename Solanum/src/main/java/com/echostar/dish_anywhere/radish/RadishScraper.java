...
public class RadishScraper {
    ...
    public List<Map<String, String>> getFilteredMovies(String genreFilter, Device device, int nMovies) {
            client = new RestClient(radishDomain);
        String json = client.Get("/v20/movies.json?&genre="+genreFilter.toLowerCase()+"&totalItems="+nMovies+"&device="+device.name()+"&itemStart=1&sort=name&uid=Q0U0QjZFQjNGOERGNUQ3NUUwNDREOEQzODVGNkMwMDY%3D--31f0f7155946fec2c9c52955302180b5b52d3110&sid=--e9ad919d527732c4bbf385ee3f5092f79d79a3cc&player_types=widevine,nagra");
        List<Map<String, String>> movies = JsonPath.read(json, "$.onlinenow.content[*]");
        return movies;
    }
